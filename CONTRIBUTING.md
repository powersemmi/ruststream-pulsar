# Contributing to ruststream-pulsar

`ruststream-pulsar` is the Apache Pulsar broker crate of RustStream. The framework's core crate lives in
...
git clone https://github.com/powersemmi/ruststream-pulsar.git
```
...
just test-brokers   # the live suite against Pulsar standalone
...
From the core, `just brokers pulsar` runs the same command against the core's working tree.
...
```bash
just brokers-up
PULSAR_TEST_URL=pulsar://127.0.0.1:6650 RUSTSTREAM_REQUIRE_LIVE=1 \
