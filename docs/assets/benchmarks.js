 ...
 * publisher, and the whole service - and the two differences between them. The code table is the
...
(() => {
  ...
  function scenarios(results, labels, lang) {
    ...
    for (const scenario of results.scenarios) {
      ...
      // A row the transport paced is marked once, on its name: both differences on it are lower
      // bounds rather than measurements, and saying so twice says it no better.
      ...
      row.appendChild(
        text("td", overhead(scenario.overhead_percent, scenario.verdict, labels)),
      );
    }
    return element;
  }
