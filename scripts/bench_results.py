...
the code-cost benches under `crates/ruststream-pulsar-bench/benches`, one JSON object per
...
FEATURES = "ruststream-pulsar default (none), ruststream macros,json"
...
def round_trip(summary: dict) -> str:
    """What one client-to-broker round trip took, as the benchmark's own probe measured it.

    Published because `broker_bound` is arithmetic over it: a reader who wants to check whether a
    row was paced by the transport needs the number the check was made against.
    """
    micros = summary.get("round_trip_micros")
    return f"{micros / 1000:.3f} ms round trip" if isinstance(micros, (int, float)) else "unknown"
...
CODE_SCENARIOS = [
    ("shared subscription, JSON decode into a small struct, ack each", "consume/service", True),
    ("reply through this crate's publisher, broker receipt awaited", "reply/service", True),
    ("shared subscription in batches of 64, assembled on the client", "batch/service", True),
]
