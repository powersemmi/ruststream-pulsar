//! Subscription registry and fanout for the in-process Pulsar stand-in.
//!
//! Core routing only: an exact-name match fans a published message out to every live
//! subscription on that name, and a per-name log records traffic for assertions. Pulsar's own
//! semantics (subscription types, dead-letter policies, ack timeouts, key sharing) are
